...
export class PartidoService {
  ...
  private partidosUrl = 'api/partidos';  // URL to web api
  ...
  getPartidos(): Observable<Partido[]> {
    ...
    getPartidosEquipo(id_equipo: number): Observable<Partido[]> {
      const url = `${this.partidosUrl}/?id_equipo=${id_equipo}`;
  ...
  addPartido(Partido: Partido): Observable<Partido> {
    return this.http.post<Partido>(this.partidosUrl, Partido, this.httpOptions).pipe(
      tap((newPartido: Partido) => this.log(`added Partido w/ id=${newPartido.id}`)),
